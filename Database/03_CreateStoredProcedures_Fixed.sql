...
BEGIN
    ...
    SELECT 
        ...
        CAST(ROUND((SUM(z.Kolicina) * a.CenaKupovine) * 100 / @UkupnaVrednost, 2) AS DECIMAL(5,2)) AS ProcenatVrednosti,
        CASE 
            WHEN (SUM(z.Kolicina) * a.CenaKupovine * 100 / @UkupnaVrednost) >= 80 THEN 'A - Kriticna'
            WHEN (SUM(z.Kolicina) * a.CenaKupovine * 100 / @UkupnaVrednost) >= 50 THEN 'B - Vazna'
            ELSE 'C - Ostalo'
        END AS Kategorija
    FROM Zalihe z
    INNER JOIN Artikli a ON z.ArtikalID = a.ArtikalID
